...
class AlgorithmBase(ABC):
    ...
    def __init__(self, config_path):
        ...
        self.OBS_DIM = 57          # 根据你的模型输入调整

        # 初始化变量
        self.model = None
        self.encoder = None
        self.encoder_hidden_state = None
        self.encoder_cell_state = None
        self.model_hidden_state = None
        self.model_cell_state = None
    ...
    def _load_policy(self):
        ...
        model_path = self.config['policy']['model_path']
        encode_path = self.config['policy']['encoder_path']
        
        # 处理相对路径
        if not os.path.isabs(model_path):
            model_path = os.path.join(self.config_dir, model_path)

        if not os.path.isabs(encode_path):
            encode_path = os.path.join(self.config_dir, encode_path)
        
        if not os.path.exists(model_path):
            raise FileNotFoundError(f"Model file not found: {model_path}")

        if not os.path.exists(encode_path):
            raise FileNotFoundError(f"Model file not found: {encode_path}")
        
        print(f"[{self.__class__.__name__}] Loading policy from: {model_path}")
        print(f"[{self.__class__.__name__}] Loading policy from: {encode_path}")
    ...
    def _warmup_policy(self):
        ...
        if self.encoder is None:
            print(f"[{self.__class__.__name__}] Warning: Encoder is None, cannot warmup")
            return
        
        if self.model is None:
            print(f"[{self.__class__.__name__}] Warning: Model is None, cannot warmup")
            return
    ...
    def _load_onnx_metadata(self, encode_path, model_path):
        """从ONNX模型metadata读取配置"""
        try:
            encoder = onnx.load(encode_path)
    ...
    def _init_model_params(self):
        ...
        if 'num_actions' not in self.model_params:
            self.model_params['num_actions'] = self.TOTAL_JOINT_DIM
        if 'num_obs' not in self.model_params:
            self.model_params['num_obs'] = 57 #114
    ...
    def split_main_and_wheel(self, action):
        """
        将 16 维动作拆成 12 维主关节 + 4 维轮子。
        兼容 12 维输入：轮子自动清零。
        """
        action = np.asarray(action, dtype=np.float32).flatten()
        if action.size >= self.TOTAL_JOINT_DIM:
            return action[:self.MAIN_JOINT_DIM], action[self.MAIN_JOINT_DIM:self.TOTAL_JOINT_DIM]
        if action.size >= self.MAIN_JOINT_DIM:
            main = action[:self.MAIN_JOINT_DIM]
            wheel = np.zeros(self.SUPPLEMENT_DIM, dtype=np.float32)
            return main, wheel
        padded = np.zeros(self.TOTAL_JOINT_DIM, dtype=np.float32)
        padded[:action.size] = action
        return padded[:self.MAIN_JOINT_DIM], padded[self.MAIN_JOINT_DIM:self.TOTAL_JOINT_DIM]

    def build_development_command(self, joint_positions, joint_velocities=None,
                                  joint_torques=None, joint_kp=None, joint_kd=None):
        """
        统一构造开发模式命令，默认支持 12+4 轴。
        """
        joint_positions = np.asarray(joint_positions, dtype=np.float32).flatten()
        main_q, wheel_q = self.split_main_and_wheel(joint_positions)

        def _prep(v, fill=0.0):
            if v is None:
                return np.full(self.TOTAL_JOINT_DIM, fill, dtype=np.float32)
            v = np.asarray(v, dtype=np.float32).flatten()
            if v.size >= self.TOTAL_JOINT_DIM:
                return v[:self.TOTAL_JOINT_DIM]
            out = np.full(self.TOTAL_JOINT_DIM, fill, dtype=np.float32)
            out[:v.size] = v
            return out

        qd = _prep(joint_velocities)
        tau = _prep(joint_torques)
        kp = _prep(joint_kp)
        kd = _prep(joint_kd)

        return {
            "enable_development_mode": True,
            "joint_positions": main_q,
            "joint_positions_supplement": wheel_q,
            "joint_velocities": qd[:self.MAIN_JOINT_DIM],
            "joint_velocities_supplement": qd[self.MAIN_JOINT_DIM:],
            "joint_torques": tau[:self.MAIN_JOINT_DIM],
            "joint_torques_supplement": tau[self.MAIN_JOINT_DIM:],
            "joint_kp": kp[:self.MAIN_JOINT_DIM],
            "joint_kp_supplement": kp[self.MAIN_JOINT_DIM:],
            "joint_kd": kd[:self.MAIN_JOINT_DIM],
            "joint_kd_supplement": kd[self.MAIN_JOINT_DIM:],
        }

    def _reset_inference_states(self):
        """重置 encoder/actor 的循环状态。"""
        self.encoder_hidden_state = None
        self.encoder_cell_state = None
        self.model_hidden_state = None
        self.model_cell_state = None
    ...
    def _run_inference(self, obs):
        """运行模型推理"""
        if (self.encoder is None)|(self.model is None):
            return None
        ...
        try:
            ...
            if len(encoder_inputs) != 3:
                raise ValueError("Encoder must have exactly 3 inputs")

            obs_info = encoder_inputs[0]
            en_hidden_info = encoder_inputs[1]
            en_cell_info = encoder_inputs[2]

            obs_name = obs_info.name
            en_hidden_name = en_hidden_info.name
            en_cell_name = en_cell_info.name

            # 自动读取 shape
            obs_dim = obs_info.shape[-1]
            encoder_hidden_dim = en_hidden_info.shape[-1]
            encoder_cell_dim = en_cell_info.shape[-1]



            # 输入
            obs_input = np.array(obs, dtype=np.float32).reshape(1, obs_dim)
            if self.encoder_hidden_state is None or self.encoder_hidden_state.shape != (1, 1, encoder_hidden_dim):
                self.encoder_hidden_state = np.zeros((1, 1, encoder_hidden_dim), dtype=np.float32)
            if self.encoder_cell_state is None or self.encoder_cell_state.shape != (1, 1, encoder_cell_dim):
                self.encoder_cell_state = np.zeros((1, 1, encoder_cell_dim), dtype=np.float32)

            encoder_outputs = self.encoder.run(
                None,
                {
                    obs_name: obs_input,
                    en_hidden_name: self.encoder_hidden_state,
                    en_cell_name: self.encoder_cell_state
                }
            )

            latent = encoder_outputs[0]
            self.encoder_hidden_state = np.asarray(encoder_outputs[1], dtype=np.float32)
            self.encoder_cell_state = np.asarray(encoder_outputs[2], dtype=np.float32)


            # model run inference
            model_inputs = self.model.get_inputs()
            if len(model_inputs) != 4:
                raise ValueError("Model must have exactly 4 inputs")

            obs_info = model_inputs[0]
            latent_info = model_inputs[1]
            hidden_info = model_inputs[2]
            cell_info = model_inputs[3]

            obs_name = obs_info.name
            latent_name = latent_info.name
            hidden_name = hidden_info.name
            cell_name = cell_info.name

            # 自动读取 shape
            obs_dim = obs_info.shape[-1]
            latent_dim = latent_info.shape[-1]
            hidden_dim = hidden_info.shape[-1]
            cell_dim = cell_info.shape[-1]
            ...
            # 输入
            obs_input = np.array(obs, dtype=np.float32).reshape(1, obs_dim)
            latent_input = np.array(latent, dtype=np.float32).reshape(1, latent_dim)
            if self.model_hidden_state is None or self.model_hidden_state.shape != (1, 1, hidden_dim):
                self.model_hidden_state = np.zeros((1, 1, hidden_dim), dtype=np.float32)
            if self.model_cell_state is None or self.model_cell_state.shape != (1, 1, cell_dim):
                self.model_cell_state = np.zeros((1, 1, cell_dim), dtype=np.float32)

            # 推理
            model_outputs = self.model.run(None, {
                obs_name: obs_input,
                latent_name: latent_input,
                hidden_name: self.model_hidden_state,
                cell_name: self.model_cell_state
            })

            if len(model_outputs) >= 3:
                self.model_hidden_state = np.asarray(model_outputs[1], dtype=np.float32)
                self.model_cell_state = np.asarray(model_outputs[2], dtype=np.float32)

            action = model_outputs[0].flatten().astype(np.float32)
